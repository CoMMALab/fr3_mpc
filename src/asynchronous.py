...
"""
Forked from Hydrax's asynchronous simulation.
...
class SharedMemoryMujocoData:
    ...
    def __init__(self, mj_data: mujoco.MjData, ctx: mp.context.BaseContext):
        ...
        self.tk = SharedMemoryNumpyArray(
            np.zeros((4,), dtype=np.float32), ctx
        )
        self.knots = SharedMemoryNumpyArray(
            np.zeros((4, mj_data.ctrl.shape[0]), dtype=np.float32), ctx
        )
        self.ctrl_start = SharedMemoryNumpyArray(
            np.zeros((1,), dtype=np.float64), ctx
        )
...
def run_controller(
    ...
    shm_data.ctrl_start[:] = np.array([start_time])
    while not finished.is_set():
        ...
        # Do a planning step
        # Advances the time of knots (tk) forward by `curr_time - start_time`
        policy_params = jit_optimize(mjx_data, policy_params)

        # Simulate random heavy-tail latency
        latency = np.random.lognormal()*0.03
        time.sleep(latency)
        print(f"Random delay: {latency}sec")

        # Send control output to the simulator,
        # but sometimes drop updates
        if np.random.uniform() > 0.1:
            # Legacy: Send the action to the simulator.
            shm_data.ctrl[:] = np.array(
                get_action(policy_params, mjx_data.time), dtype=np.float32
            )

            # Better: Get time _after_ calling optimize.
            # shm_data.ctrl[:] = np.array(
            #     get_action(policy_params, time.time() - start_time), dtype=np.float32
            # )

            # Best: Send spline parameters directly.
            shm_data.tk[:] = np.array(policy_params.tk, dtype=np.float32)
            shm_data.knots[:] = np.array(policy_params.mean, dtype=np.float32)
...
def run_simulator(
    ...
    interp_func = jax.jit(interp_func)

    recorder = None
    actual_frequency = 1.0 / mj_model.opt.timestep
    if record_video:
        # Video dimensions
    ...
    with mujoco.viewer.launch_passive(mj_model, mj_data) as viewer:
        while viewer.is_running():
            ...
            if query_spline:
                # Actually query the spline rather than assuming zero-order
                # hold and a sufficiently high control rate
                tk = shm_data.tk[:]
                mj_data.ctrl[:] = np.array(
                    interp_func(time.time() - shm_data.ctrl_start[:][0], tk, shm_data.knots[:][None,...])[0], dtype=np.float32
                )
            else:
...
def run_interactive(
    ...
    sim = ctx.Process(
        ...
        args=(mj_model, mj_data, shm_data, controller.interp_func, ready, finished, record_video),
    )
    control = ctx.Process(
